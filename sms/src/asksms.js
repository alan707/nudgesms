...
const askSMS = function*(next){
  ...
  for(var sub of phoneres) {
    let queryurl = `https://greengov.data.ca.gov/resource/5pvh-h9tc.json?$limit=30&property_id=${sub.building}&$order=year_ending&$where=year_ending>'2014-07-04' AND starts_with(use_metrics, '${metric}')`;
    ...
    messagequeue.push(this.smsClient.sendMessage({
      ...
      body: `Sure thing, your building used up ${kw} ${units} last week. That's 10% higher than the same week last year.`
    }));
  }
  try{
